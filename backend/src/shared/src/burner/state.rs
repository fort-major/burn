...
use super::{
    api::{GetBalanceRequest, GetBalanceResponse, GetTotalsResponse},
    types::{BurnerStateInfo, Memory, TCycles, TCYCLE_POS_ROUND_FEE},
};
...
impl BurnerState {
    ...
    pub fn claim_reward(&mut self, caller: Principal) -> Option<E8s> {
        let fee = TCycles::from(TCYCLE_POS_ROUND_FEE);
    ...
    pub fn pos_round_batch(&mut self, batch_size: u64) -> bool {
        ...
        let mut iter = if let Some(id) = info.next_burner_id {
            self.shares.iter_upper_bound(&id)
        } else {
            self.shares.iter()
        };
    ...
    pub fn get_shares(&self, req: GetBalanceRequest) -> GetBalanceResponse {
        let entries = req
            .ids
            .into_iter()
            .map(|id| self.shares.get(&id).map(|it| it.0).unwrap_or_default());
    ...
    pub fn get_totals(&self, caller: &Principal) -> GetTotalsResponse {
        ...
        GetTotalsResponse {
            total_tcycles_supply: info.total_shares_supply,
